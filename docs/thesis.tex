...
with one particular SA. For outbound processing, each SAD entry is pointed to by entries in the SPD cache.  For inbound processing, for unicast SAs, the SPI is used either alone to look up an SA or in conjunction with the IPsec protocol type \cite{rfc4301}.
	...
	\subsection{Internet Security Association and Key Management Protocol}
	The Internet Security Association and Key Management Protocol (ISAKMP) is responsible for establishing, negotiating, modifying and deleting Security Associations, not only for IPsec, but for many other security protocols. ISAKMP is designed to be key exchange independent, supporting any such protocol, although Internet Key Exchange (IKE) and Kerberized Internet Negotiation of Keys (KINK) are the most common.
	
	ISAKMP defines procedures and packet formats which are used to transfer information such as security association data, or key exchange data, in order to establish a security association between negotiating entities. This is done in two phases:
	\begin{itemize}
		\item In the first phase, two entities (e.g.  ISAKMP servers) agree on how to protect further negotiation traffic between themselves, establishing an ISAKMP SA.
		\item In the second phase, this ISAKMP SA is then used to protect the negotiations for the Protocol SA being requested.
	\end{itemize}
	
	While bootstrapping secure channels between systems, ISAKMP cannot assume the existence of security services, and must provide some protections for itself.  Therefore, ISAKMP considers an ISAKMP Security Association to be different than other types, and manages ISAKMP SAs itself, in their own name space \cite{rfc2408}. In order to identify ISAKMP SAs, two cookie fields in the ISAKMP header are used. These cookies are usually a hash over the IP Source and Destination Address, the UDP Source and Destination Ports, a locally generated secret random value and the current time.
	
	Although ISAKMP can be implemented over any transport protocol or even over IP itself, it is usually used over UDP on port 500.
