...
\begin{document}
	...
	% Print acronyms page
	\glsaddall
	\printglossary[type=\acronymtype,nonumberlist,style=listdotted]
	...
	\section{Description }
		...
		IPsec has two main modes of operation which defined how AH and ESP behave, namely transport mode and tunnel mode. % TODO
...
with one particular SA. For outbound processing, each SAD entry is pointed to by entries in the SPD cache.  For inbound processing, for unicast SAs, the SPI is used either alone to look up an SA or in conjunction with the IPsec protocol type \cite{rfc4301}.
	...
	\begin{itemize}
	\item Next Header (8 bits) is used to link headers together and identifies the next protocol used after AH. The value is chosen from the set of IP Protocol Numbers defined by IANA. For example, a value of 4 indicates IPv4, a value of 41 indicates IPv6, and a value of 6 indicates TCP. AH itself has the value of 41 assigned.
...
\end{itemize}			
		...
		AH placement depends on the IPsec mode of operation and whether IPv4 or IPv6 is used.
